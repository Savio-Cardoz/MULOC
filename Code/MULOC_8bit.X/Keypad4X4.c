...
{
	KEYPAD_PORT_DIR = 0x0F;	// Last 4 pins (ROW) output, First 4 pins (COLOUMN) input
    KEYPAD_PORT_OUT = 0xFF;     // Initial value to wake up controller on Pin-change at the column inputs.
...
{
    keypadStatus KeypadStatus;
    getKey(&KeypadStatus);
    if(KeypadStatus.keyPressIndicator == True)
        return 1;
    else return 0;
}
